...
class BIT1D(object):
	...
	def update(self, index, val):
		if index <= 0 or index > self.maxlen: return
		x = index
		self.arr[x-1] += val
		while x <= self.maxlen:
			self.bit[x] += val
	...
	def query(self, index):
		if index < 0 or index > self.maxlen: return 0
		x = index
		out = 0
		while x > 0:
			out += self.bit[x]
...
class BIT2D(object):
	...
	def update(self, i, j, val):
		if i <= 0 or i > self.m: return
		if j <= 0 or j > self.n: return
		x = i
	...
	def rangeQuery(self, i1, j1, i2, j2):
		return self.query(i1-1, j1-1) - self.query(i1-1, j2) - self.query(i2, j1-1) + self.query(i2, j2)
...
def bit2trial():
	...
	bit2.build()
	print bit2.rangeQuery(0,0,4,3)
	print bit2.rangeQuery(3,2,4,3)
	bit2.update(4,3,2)
	print bit2.rangeQuery(3,2,4,3)
