...
class Solution(object):
    def movesToChessboard(self, board):
        ...
        if rowSum < N / 2 or rowSum > (N + 1) / 2 or colSum < N / 2 or colSum > (N + 1) / 2: return -1
        rowSwap, colSwap = sum(board[i][0] == i % 2 for i in range(N)), sum(board[0][i] == i % 2 for i in range(N))
