"""Hungarian Algorithm solution by stockfish@."""
...
class Hungarian:
    ...
    def _adjust_matrix_by_min_uncovered_num(
        ...
        m_matrix = np.ones(self._shape, dtype=np.int64) * min_uncovered_num
...
class Solution(object):
    def assignBikes(self, workers, bikes):
        ...
        for i in range(m):
            for j in range(n):
                graph[i][j] = abs(workers[i][0] - bikes[j][0]) + abs(
                    workers[i][1] - bikes[j][1]
                )
                # for i in range(n - m):
                #    graph.append([0] * n)
        hungarian = Hungarian(graph)
        hungarian.calculate()
        return sum([graph[i][j] for i, j in hungarian.get_results()])
